...
const FamilyBudget = () => {
  ...
  useEffect(() => {
    const fetchBudgetData = async () => {
      try {
        setLoading(true);
        const response = await getFamilyBudget();
        ...
        if (response.error) {
          ...
          // Extract budgets from the response
          const budgetData = response.data.budgetData || [];
          setBudgets(budgetData);
          calculateTotals(budgetData);
        }
      } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (dialogMode === "add") {
        ...
        // Add new budget via API
        const response = await addFamilyBudget({
          familyId,
        ...
        if (response.error) {
          setError(
            response.message || "Failed to add budget. Please try again."
          );
        } else {
          // Refresh budget data
          const refreshResponse = await getFamilyBudget();
          if (refreshResponse.data.budgetData) {
            setBudgets(refreshResponse.data.budgetData);
            calculateTotals(refreshResponse.data.budgetData);
          }
        }
      } else {
        // Update existing budget via API
        const response = await updateFamilyBudget({
        ...
        if (response.error) {
          setError(
            response.message || "Failed to update budget. Please try again."
          );
        } else {
          // Refresh budget data
          const refreshResponse = await getFamilyBudget();
  ...
  const handleDeleteBudget = async () => {
    ...
    try {
      const response = await deleteFamilyBudget(budgetToDelete.budgetId);

      if (response.error) {
        setError(
          response.message || "Failed to delete budget. Please try again."
        );
      } else {
        // Refresh budget data
        const refreshResponse = await getFamilyBudget();
        if (refreshResponse.data.budgetData) {
          ...
          // If no data returned, just remove from state
          const updatedBudgets = budgets.filter(
            (budget) => budget.budgetId !== budgetToDelete.budgetId
          );
          ...
          <Grid container spacing={3} sx={{ mb: 3 }}>
            <Grid item xs={12} md={6}>
              <BudgetPieChart
                ...
                subtitle="How your family budget is distributed across categories"
                data={pieChartData}
                height={300}
                cardProps={{
                  sx: { height: "100%" },
            ...
            <Box sx={{ p: 4, textAlign: "center" }}>
              <Typography color="text.secondary">
                ...
                {familyRole !== "child"
                  ...
                  : "Your family has no budget yet. Please ask your parent to create your first budget."}
              </Typography>
            </Box>
          )}
        </Paper>

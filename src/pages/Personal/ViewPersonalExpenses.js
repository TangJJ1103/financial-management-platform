...
const ViewPersonalExpenses = () => {
  ...
  const calculateStatistics = (expensesData) => {
    ...
    // Total expenses (sum of amountConverted)
  ...
  }, [expenses, searchTerm, categoryFilter, dateFilter, order, orderBy]);
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const exportData = filteredExpenses.map((expense) => ({
        ...
        "Created On": formatDate(expense.createdOn),
      }));
      ...
      // Save file
      saveAs(data, `Personal_Expenses_${dayjs().format("YYYY-MM-DD")}.xlsx`);
    } catch (error) {
  ...
  const handleDeleteExpense = async () => {
    ...
    try {
      const response = await deleteUserExpense(expenseToDelete.expensesId);
              ...
              <TableHead>
                ...
                >
                  ...
                  {!isTablet && (
                    <TableCell>
                      <TableSortLabel
                        active={orderBy === "amountEntry"}
                        direction={orderBy === "amountEntry" ? order : "asc"}
                        onClick={() => handleRequestSort("amountEntry")}
                      >
                        Original Amount
                      </TableSortLabel>
                    </TableCell>
                        ...
                        {!isTablet && (
                          <TableCell>
                            <Typography variant="body2" color="text.secondary">
                              {expense.amountEntry} {expense.currencyType}
                            </Typography>
                          </TableCell>
                        )}
                        {!isMobile && (
...
export default ViewPersonalExpenses;
